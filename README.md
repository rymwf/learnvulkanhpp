...
- **构建**：CMake 4.1+，C++23，当前配置使用 **libc++**（`-stdlib=libc++`），需与本机工具链一致。
...
使用前请安装 **Vulkan SDK** 与兼容的 **LLVM/Clang** 环境，并确保已安装验证层等可选组件以便调试。

当前程序是在msys2 ucrt clang22 环境下配置的，并没有对其他编译器做兼容性适配。
